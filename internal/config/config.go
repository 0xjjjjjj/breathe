...
func DefaultConfig() *Config {
	return &Config{
		...
		OrganizeRules: []OrganizeRule{
			{Match: "*.{pdf,doc,docx,txt,rtf,odt}", Dest: "~/Documents"},
			{Match: "*.{csv,xlsx,xls}", Dest: "~/Documents/Data"},
			{Match: "*.{json,xml}", Dest: "~/Documents/Data"},
			{Match: "*.{jpg,jpeg,png,gif,webp,svg,heic}", Dest: "~/Pictures"},
			{Match: "Screenshot*.png", Dest: "~/Pictures/Screenshots"},
			{Match: "*.{mp4,mov,avi,mkv,webm}", Dest: "~/Movies"},
			{Match: "*.{mp3,wav,flac,m4a,aac}", Dest: "~/Music"},
